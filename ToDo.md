...
# 25. 05. 15
### 진행사항
1. 토큰(글자 수)이 제한되어 있기 때문에 본문이 아닌 제목을 전송
    => 사이트 1개당 20개를 추려서 gpt에 제공 (총 80개)
2. 중간에 에러가 발생할 경우 시스템이 그대로 멈춰버림
    

### 문제
1. 링크 전달 후 본문 요약 요청 시 엉뚱한 내용으로 요약  
   => 제목과 함께 전달하여 정확도 향상
2. GPT 응답이 숫자가 아닌 문자열과 섞이는 현상이 종종 발생함  
   => 이 경우 예외 발생시 기본값을 세팅?
3. GPT 본문 요약이 너무 짧을 때가 있음  
    => 150자 이내로 요약이 아니라 "140-150자에 맞춰서 작성"으로 한다면?
***
# 25. 05. 21
### 단위 테스트
1. 지피티가 뒤의 문장까지 끝까지 읽어오는가? 에 대한 테스트 수행
